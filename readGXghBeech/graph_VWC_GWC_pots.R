...
par(mar=c(5,5,0,0))
plot(cont[[1]][,'nday'], cont[[1]][,'VWC.mean.na'], type='l', col='black', lwd=0.75, ylim=c(0, 0.57),
     xlab='Days since beginning drought treatment', ylab = '', cex.lab=1, xlim=c(0,36), axes=F)
axis(1, at=seq(0, 36, 5), labels = seq(0, 36, 5))
mtext(expression(VWC~(cm^3~cm^-3)), side = 2, line = 3, cex = 1.3)
axis(2, at=seq(0, 0.5, 0.1), labels = seq(0, 0.5, 0.1), las=1)
box()
for (i in 1:length(cont)){
  lines(cont[[i]][,'nday'], cont[[i]][,'VWC.mean.na'], lwd=0.9, col='black')
  Hmisc::errbar(cont[[i]][,'nday'], cont[[i]][,'VWC.mean.na'], cont[[i]][,'VWC.mean.na']+cont[[i]][,'VWC.s.err'],
                cont[[i]][,'VWC.mean.na'] - cont[[i]][,'VWC.s.err'], pch=21, bg=myPal[i], col='black', cex=1.4, add=T)
  points(cont[[i]][,'nday'], cont[[i]][,'VWC.mean.na'], col='black', bg=myPal[i], pch=21, cex=1.4) 
}
# legend('topleft', legend='(c) Control', text.font = 2, bty='n', cex = 1.2)

par(mar=c(5,0,0,5))

plot(dro[[1]][,'nday'], dro[[1]][,'VWC.mean.na'], type='l', col='black', lwd=0.75, ylim=c(0,0.57),
     xlab='Days since beginning drought treatment', ylab='', cex.lab=1, xlim=c(0,36), axes=F)
axis(1, at=seq(0, 36, 5), labels = seq(0, 36, 5))
axis(2, at=seq(0, 0.5, 0.1), labels = NA)
box()
for (i in 1:length(dro)){
  lines(dro[[i]][,'nday'], dro[[i]][,'VWC.mean.na'], lwd=0.9)
  Hmisc::errbar(dro[[i]][,'nday'], dro[[i]][,'VWC.mean.na'], dro[[i]][,'VWC.mean.na']+dro[[i]][,'VWC.s.err'],
                dro[[i]][,'VWC.mean.na'] - dro[[i]][,'VWC.s.err'], pch=25, bg=myPal[i], col='black', cex=1.2, add=T)
  points(dro[[i]][,'nday'], dro[[i]][,'VWC.mean.na'], col='black', bg=myPal[i], pch=25, cex=1.2) 
}
